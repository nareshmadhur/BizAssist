...
import { getClientUseCases } from "@/app/lib/client-storage";

interface PortfolioViewProps {
    serverData: UseCase[];
}

export default function PortfolioView({ serverData }: PortfolioViewProps) {
    ...
    useEffect(() => {
        ...
        if (local.length === 0 && serverData.length > 0) {
            ...
            // We rely on the dashboard to seed the local storage or just mix them here?
            // If the user hasn't visited dashboard, local might be empty.
            // Let's just use serverData if local is empty.
        } else {
            setUseCases(local);
        }
        setLoading(false);
